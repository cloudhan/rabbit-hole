...
template <>
struct microkernel<8, 4> {
  constexpr static size_t MR = 8;
  constexpr static size_t NR = 4;
...
template <>
struct microkernel<16, 4> {
  constexpr static size_t MR = 16;
  constexpr static size_t NR = 4;

  void operator()(int k, const float *a, int lda, const float *b, int ldb, float *c, int ldc) const {
    // Declare vector registers to hold 16x4 C and load them
    __m256 c_01234567_0 = _mm256_loadu_ps(&C(0, 0));
