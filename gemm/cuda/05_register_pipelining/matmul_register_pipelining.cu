...
MATMUL_KERNEL_SIGNATURE(matmul_kernel_register_pipelining) {
  ...
  for (int p = 0; p < k; p += SmemShapeK) {
    ...
    load_fragment<ThreadShapeM, SmemShapeK, SmemShapeM>(frag_a[0], smem_a, 0, smem_A_thread_i);
    load_fragment<ThreadShapeN, SmemShapeK, SmemShapeN>(frag_b[0], smem_b, 0, smem_B_thread_j);
#pragma unroll
    for (int smem_AB_thread_p = 0; smem_AB_thread_p < SmemShapeK; smem_AB_thread_p++) {
      ...
      if (smem_AB_thread_p + 1 < SmemShapeK) {
        load_fragment<ThreadShapeM, SmemShapeK, SmemShapeM>(frag_a[(smem_AB_thread_p + 1) % 2], smem_a, smem_AB_thread_p + 1, smem_A_thread_i);
        load_fragment<ThreadShapeN, SmemShapeK, SmemShapeN>(frag_b[(smem_AB_thread_p + 1) % 2], smem_b, smem_AB_thread_p + 1, smem_B_thread_j);
      }
