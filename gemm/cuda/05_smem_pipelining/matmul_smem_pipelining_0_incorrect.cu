...
MATMUL_KERNEL_SIGNATURE(matmul_kernel_smem_pipelining_incorrect) {
  ...
  store_smem_load_global_a<NumThreads, SmemShapeM, SmemShapeK>(smem_a[0], m, k, a, lda, 0);
  store_smem_load_global_b<NumThreads, SmemShapeK, SmemShapeN>(smem_b[0], k, n, b, ldb, 0);
  for (int p = 0; p < k; p += SmemShapeK) {
    __syncthreads();
    if (p + SmemShapeK < k) {
      store_smem_load_global_a<NumThreads, SmemShapeM, SmemShapeK>(smem_a[((p / SmemShapeK) + 1) % 2], m, k, a, lda, p+SmemShapeK);
      store_smem_load_global_b<NumThreads, SmemShapeK, SmemShapeN>(smem_b[((p / SmemShapeK) + 1) % 2], k, n, b, ldb, p+SmemShapeK);
    }
