...
MATMUL_KERNEL_SIGNATURE(matmul_kernel_smem_pipelining_1_clobbered) {
  ...
  for (int p = 0; p < k; p += SmemShapeK) {
    ...
    for (int smem_AB_thread_p = 0; smem_AB_thread_p < SmemShapeK; smem_AB_thread_p++) {
      // register load
      load_fragment<ThreadShapeM, SmemShapeK, SmemShapeM>(frag_a, smem_a[(p / SmemShapeK) % 2], smem_AB_thread_p, smem_A_thread_i);
      load_fragment<ThreadShapeN, SmemShapeK, SmemShapeN>(frag_b, smem_b[(p / SmemShapeK) % 2], smem_AB_thread_p, smem_B_thread_j);
...
MATMUL_KERNEL_LAUNCH(matmul_kernel_smem_pipelining_1_clobbered, 256, 128, 128, 8, 8, 8);
MATMUL_KERNEL_LAUNCH(matmul_kernel_smem_pipelining_1_clobbered, 256, 128, 128, 4, 8, 8);
MATMUL_KERNEL_LAUNCH(matmul_kernel_smem_pipelining_1_clobbered, 256, 128, 128, 16, 8, 8);
MATMUL_KERNEL_LAUNCH(matmul_kernel_smem_pipelining_1_clobbered, 256, 128, 128, 24, 8, 8);

MATMUL_DMODULE(m) {
  REGISTER(launch_matmul_kernel_smem_pipelining_1_clobbered_256t_cta128x128_smem8_thread8x8);
  REGISTER(launch_matmul_kernel_smem_pipelining_1_clobbered_256t_cta128x128_smem4_thread8x8);
  REGISTER(launch_matmul_kernel_smem_pipelining_1_clobbered_256t_cta128x128_smem16_thread8x8);
  REGISTER(launch_matmul_kernel_smem_pipelining_1_clobbered_256t_cta128x128_smem24_thread8x8);
}
