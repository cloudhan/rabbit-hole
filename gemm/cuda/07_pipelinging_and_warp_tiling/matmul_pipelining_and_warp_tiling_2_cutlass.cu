...
MATMUL_KERNEL_SIGNATURE(matmul_kernel_pipelining_and_warp_tiling_2_cutlass) {
  ...
  PRINTF("%4d s->r    0    0\n", __LINE__);
  load_fragment<ThreadShapeM, SmemShapeK, SmemShapeM>(frag_a[0], smem_a[0], 0, smem_A_thread_i);
  load_fragment<ThreadShapeN, SmemShapeK, SmemShapeN>(frag_b[0], smem_b[0], 0, smem_B_thread_j);
...
#pragma unroll
    for (int smem_AB_thread_p = 0; smem_AB_thread_p < SmemShapeK; smem_AB_thread_p++) {
      ...
      load_fragment<ThreadShapeM, SmemShapeK, SmemShapeM>(frag_a[(smem_AB_thread_p + 1) % 2], smem_a[p_tile_curr % 2], (smem_AB_thread_p + 1) % SmemShapeK, smem_A_thread_i);
      load_fragment<ThreadShapeN, SmemShapeK, SmemShapeN>(frag_b[(smem_AB_thread_p + 1) % 2], smem_b[p_tile_curr % 2], (smem_AB_thread_p + 1) % SmemShapeK, smem_B_thread_j);
...
MATMUL_KERNEL_LAUNCH(matmul_kernel_pipelining_and_warp_tiling_2_cutlass, 256, 128, 128, 8, 64, 32, 8, 8);
MATMUL_KERNEL_LAUNCH(matmul_kernel_pipelining_and_warp_tiling_2_cutlass, 256, 128, 128, 8, 32, 64, 8, 8);
MATMUL_KERNEL_LAUNCH(matmul_kernel_pipelining_and_warp_tiling_2_cutlass, 256, 128, 128, 16, 64, 32, 8, 8);
MATMUL_KERNEL_LAUNCH(matmul_kernel_pipelining_and_warp_tiling_2_cutlass, 256, 128, 128, 16, 32, 64, 8, 8);

MATMUL_DMODULE(m) {
  REGISTER(launch_matmul_kernel_pipelining_and_warp_tiling_2_cutlass_256t_cta128x128_smem8_warp64x32_thread8x8);
  REGISTER(launch_matmul_kernel_pipelining_and_warp_tiling_2_cutlass_256t_cta128x128_smem8_warp32x64_thread8x8);
  REGISTER(launch_matmul_kernel_pipelining_and_warp_tiling_2_cutlass_256t_cta128x128_smem16_warp64x32_thread8x8);
  REGISTER(launch_matmul_kernel_pipelining_and_warp_tiling_2_cutlass_256t_cta128x128_smem16_warp32x64_thread8x8);
}
