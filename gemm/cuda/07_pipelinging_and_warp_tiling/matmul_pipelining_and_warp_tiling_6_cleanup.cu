...
__device__ void load_global_a(
  ...
  const int A_batchp = a_cta_p + threadIdx.x / SmemShapeM;
  auto A_p = A_batchp;
#pragma unroll
...
__device__ void load_global_b(
  ...
  const int B_p = b_cta_p + threadIdx.x % SmemShapeK;
...
#pragma unroll
  for (int f = 0; f < FragmentSize; f += 4, ptr += Step) {
    ...
    lds128(frag_a[f + 0], frag_a[f + 1], frag_a[f + 2], frag_a[f + 3], ptr);
  }
}
...
MATMUL_KERNEL_SIGNATURE(matmul_kernel_pipelining_and_warp_tiling_6) {
...
MATMUL_KERNEL_LAUNCH(matmul_kernel_pipelining_and_warp_tiling_6, 256, 128, 128, 8, 64, 32, 8, 8);
MATMUL_KERNEL_LAUNCH(matmul_kernel_pipelining_and_warp_tiling_6, 256, 128, 128, 8, 32, 64, 8, 8);
MATMUL_KERNEL_LAUNCH(matmul_kernel_pipelining_and_warp_tiling_6, 256, 128, 128, 16, 64, 32, 8, 8);
MATMUL_KERNEL_LAUNCH(matmul_kernel_pipelining_and_warp_tiling_6, 256, 128, 128, 16, 32, 64, 8, 8);

MATMUL_DMODULE(m) {
  REGISTER(launch_matmul_kernel_pipelining_and_warp_tiling_6_256t_cta128x128_smem8_warp64x32_thread8x8);
  REGISTER(launch_matmul_kernel_pipelining_and_warp_tiling_6_256t_cta128x128_smem8_warp32x64_thread8x8);
  REGISTER(launch_matmul_kernel_pipelining_and_warp_tiling_6_256t_cta128x128_smem16_warp64x32_thread8x8);
  REGISTER(launch_matmul_kernel_pipelining_and_warp_tiling_6_256t_cta128x128_smem16_warp32x64_thread8x8);
}
