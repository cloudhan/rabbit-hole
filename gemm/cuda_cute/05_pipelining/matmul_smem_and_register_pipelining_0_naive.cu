...
__device__ void load_global_a(RegT& rA, const GmemT& gA, CoordT cA, int m, int k) {
...
__device__ void load_global_b(RegT& rB, const GmemT& gB, CoordT cB, int n, int k) {
  static_assert(is_rmem<typename RegT::engine_type>());
  static_assert(VecSize == 1 || VecSize == 2 || VecSize == 4);
...
__launch_bounds__(NumThreads, 2) MATMUL_KERNEL_SIGNATURE(matmul_smem_and_register_pipelining_0) {
  ...
  // double buffering
  constexpr const auto SmemALayout = make_layout(make_shape(Int<SmemShapeM>{}, Int<SmemShapeK>{}, _2{}), make_stride(_1{}, Int<SmemShapeM + SmemAPad>{}, Int<(((SmemShapeM + SmemAPad) * SmemShapeK - 1) / 32  + 1) * 32>{}));
  constexpr const auto SmemBLayout = make_layout(make_shape(Int<SmemShapeN>{}, Int<SmemShapeK>{}, _2{}), make_stride(_1{}, Int<SmemShapeN + SmemBPad>{}, Int<(((SmemShapeN + SmemBPad) * SmemShapeK - 1) / 32  + 1) * 32>{}));
  ...
  decltype(make_fragment_like<float>(Int<ThreadShapeM>{})) fragA[2] = {make_fragment_like<float>(Int<ThreadShapeM>{}), make_fragment_like<float>(Int<ThreadShapeM>{})};
  decltype(make_fragment_like<float>(Int<ThreadShapeN>{})) fragB[2] = {make_fragment_like<float>(Int<ThreadShapeN>{}), make_fragment_like<float>(Int<ThreadShapeN>{})};
...
#pragma unroll 1  // no unroll
  for (int block_p = 0; block_p < num_smem_block; block_p++) {
    const auto curr_smem_idx = block_p % 2;
    store_smem_a<NumThreads, SmemShapeM, SmemShapeK, SmemALoadStoreVec>(sA(_, _, curr_smem_idx), staging_a, m, k);
    store_smem_b<NumThreads, SmemShapeN, SmemShapeK, SmemBLoadStoreVec>(sB(_, _, curr_smem_idx), staging_b, n, k);
    __syncthreads();
    if (block_p + 1 < num_smem_block) {
      load_global_a<NumThreads, SmemShapeM, SmemShapeK, SmemALoadStoreVec>(staging_a, stripe_gA(_, _, _0{}, block_p + 1), stripe_cA(_, _, _0{}, block_p + 1), m, k);
      load_global_b<NumThreads, SmemShapeN, SmemShapeK, SmemBLoadStoreVec>(staging_b, stripe_gB(_, _, _0{}, block_p + 1), stripe_cB(_, _, _0{}, block_p + 1), n, k);
    }

    copy(stripe_sA(_, 0, _0{}, curr_smem_idx), fragA[0]);  // load_fragment a
    copy(stripe_sB(_, 0, _0{}, curr_smem_idx), fragB[0]);  // load_fragment b
#pragma unroll
    for (int smem_AB_thread_p = 0; smem_AB_thread_p < SmemShapeK - 1; smem_AB_thread_p++) {
      const auto next_frag_idx = (smem_AB_thread_p + 1) % 2;
      copy(stripe_sA(_, smem_AB_thread_p+1, _0{}, curr_smem_idx), fragA[next_frag_idx]);  // load_fragment a
      copy(stripe_sB(_, smem_AB_thread_p+1, _0{}, curr_smem_idx), fragB[next_frag_idx]);  // load_fragment b
      const auto curr_frag_idx = smem_AB_thread_p % 2;
      rank1_update(fragA[curr_frag_idx], fragB[curr_frag_idx], acc);
    }
    const auto curr_frag_idx = (SmemShapeK - 1) % 2;
    rank1_update(fragA[curr_frag_idx], fragB[curr_frag_idx], acc);
  }
...
MATMUL_KERNEL_LAUNCH(matmul_smem_and_register_pipelining_0, 256, 128, 128, 4, 8, 8);
MATMUL_KERNEL_LAUNCH(matmul_smem_and_register_pipelining_0, 256, 128, 128, 8, 8, 8);
// MATMUL_KERNEL_LAUNCH(matmul_smem_and_register_pipelining_0, 256, 128, 128, 16, 8, 8);
// MATMUL_KERNEL_LAUNCH(matmul_smem_and_register_pipelining_0, 256, 128, 128, 24, 8, 8);
// MATMUL_KERNEL_LAUNCH(matmul_smem_and_register_pipelining_0, 256, 128, 128, 32, 8, 8);
// MATMUL_KERNEL_LAUNCH(matmul_smem_and_register_pipelining_0, 256, 128, 128, 40, 8, 8);
// MATMUL_KERNEL_LAUNCH(matmul_smem_and_register_pipelining_0, 256, 128, 128, 48, 8, 8);

MATMUL_DMODULE(m) {
  REGISTER(launch_matmul_smem_and_register_pipelining_0_256t_cta128x128_smem4_thread8x8);
  REGISTER(launch_matmul_smem_and_register_pipelining_0_256t_cta128x128_smem8_thread8x8);
  // REGISTER(launch_matmul_smem_and_register_pipelining_0_256t_cta128x128_smem16_thread8x8);
  // REGISTER(launch_matmul_smem_and_register_pipelining_0_256t_cta128x128_smem24_thread8x8);
  // REGISTER(launch_matmul_smem_and_register_pipelining_0_256t_cta128x128_smem32_thread8x8);
  // REGISTER(launch_matmul_smem_and_register_pipelining_0_256t_cta128x128_smem40_thread8x8);
  // REGISTER(launch_matmul_smem_and_register_pipelining_0_256t_cta128x128_smem48_thread8x8);
}
