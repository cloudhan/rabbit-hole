...
MATMUL_KERNEL_SIGNATURE(matmul_smem_and_register_pipelining_1) {
  ...
  store_smem_a<NumThreads, SmemShapeM, SmemShapeK, SmemALoadStoreVec>(sA(_, _, _0{}), staging_a, m, k);
...
#pragma unroll
    for (int smem_AB_thread_p = 0; smem_AB_thread_p < SmemShapeK; smem_AB_thread_p++) {
      if (smem_AB_thread_p == SmemShapeK - 1) {
        store_smem_a<NumThreads, SmemShapeM, SmemShapeK, SmemALoadStoreVec>(sA(_, _, p_tile_next % 2), staging_a, m, k);
        store_smem_b<NumThreads, SmemShapeN, SmemShapeK, SmemBLoadStoreVec>(sB(_, _, p_tile_next % 2), staging_b, n, k);
        __syncthreads();
        if (--p_tile_count > 0) {
          ++p_tile_next;
        }
      ...
      copy(stripe_sA(_, (smem_AB_thread_p + 1) % SmemShapeK, _0{}, p_tile_curr % 2), fragA[(smem_AB_thread_p + 1) % 2]);  // load_fragment a
      copy(stripe_sB(_, (smem_AB_thread_p + 1) % SmemShapeK, _0{}, p_tile_curr % 2), fragB[(smem_AB_thread_p + 1) % 2]);  // load_fragment b
...
MATMUL_KERNEL_LAUNCH(matmul_smem_and_register_pipelining_1, 256, 128, 128, 4, 8, 8);
MATMUL_KERNEL_LAUNCH(matmul_smem_and_register_pipelining_1, 256, 128, 128, 8, 8, 8);
// MATMUL_KERNEL_LAUNCH(matmul_smem_and_register_pipelining_1, 256, 128, 128, 16, 8, 8);
// MATMUL_KERNEL_LAUNCH(matmul_smem_and_register_pipelining_1, 256, 128, 128, 24, 8, 8);
// MATMUL_KERNEL_LAUNCH(matmul_smem_and_register_pipelining_1, 256, 128, 128, 32, 8, 8);
// MATMUL_KERNEL_LAUNCH(matmul_smem_and_register_pipelining_1, 256, 128, 128, 40, 8, 8);
// MATMUL_KERNEL_LAUNCH(matmul_smem_and_register_pipelining_1, 256, 128, 128, 48, 8, 8);

MATMUL_DMODULE(m) {
  REGISTER(launch_matmul_smem_and_register_pipelining_1_256t_cta128x128_smem4_thread8x8);
  REGISTER(launch_matmul_smem_and_register_pipelining_1_256t_cta128x128_smem8_thread8x8);
  // REGISTER(launch_matmul_smem_and_register_pipelining_1_256t_cta128x128_smem16_thread8x8);
  // REGISTER(launch_matmul_smem_and_register_pipelining_1_256t_cta128x128_smem24_thread8x8);
  // REGISTER(launch_matmul_smem_and_register_pipelining_1_256t_cta128x128_smem32_thread8x8);
  // REGISTER(launch_matmul_smem_and_register_pipelining_1_256t_cta128x128_smem40_thread8x8);
  // REGISTER(launch_matmul_smem_and_register_pipelining_1_256t_cta128x128_smem48_thread8x8);
}
