...
__launch_bounds__(NumThreads, 2)
    MATMUL_KERNEL_SIGNATURE(matmul_kernel_copy_0_sts) {
  ...
  constexpr const auto SmemALoadStoreVec = (SmemShapeM * SmemShapeK) / NumThreads;
  constexpr const auto SmemBLoadStoreVec = (SmemShapeN * SmemShapeK) / NumThreads;
  const auto tiled_copy_smem_a = make_tiled_copy(
      Copy_Atom<DefaultCopy, float>{},
      make_layout(make_shape(Int<SmemShapeM/SmemALoadStoreVec>{}, Int<SmemShapeK>{})),
      make_layout(make_shape(Int<SmemALoadStoreVec>{}))
  );
  const auto tiled_copy_smem_b = make_tiled_copy(
      Copy_Atom<DefaultCopy, float>{},
      make_layout(make_shape(Int<SmemShapeN / SmemBLoadStoreVec>{}, Int<SmemShapeK>{}), LayoutRight{}),
...
MATMUL_KERNEL_LAUNCH(matmul_kernel_copy_0_sts, 256, 128, 128, 8, 32, 64, 8, 8);
MATMUL_KERNEL_LAUNCH(matmul_kernel_copy_0_sts, 256, 128, 128, 8, 64, 32, 8, 8);

MATMUL_DMODULE(m) {
  REGISTER(launch_matmul_kernel_copy_0_sts_256t_cta128x128_smem8_warp32x64_thread8x8);
  REGISTER(launch_matmul_kernel_copy_0_sts_256t_cta128x128_smem8_warp64x32_thread8x8);
}
