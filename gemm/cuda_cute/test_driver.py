...
import sys
if os.name == "posix":
  if not os.path.exists(os.path.join(os.path.dirname(__file__), "../linux/bazel-bin/cuda_cute/matmul.so")):
    raise EnvironmentError("bazel build -c opt --config=linux '//cuda_cute:matmul.so'")
  sys.path.append(os.path.join(os.path.dirname(__file__), "../linux/bazel-bin/cuda_cute"))
if os.name == "nt":
  if not os.path.exists(os.path.join(os.path.dirname(__file__), "../windows/bazel-bin/cuda_cute/matmul.pyd")):
    raise EnvironmentError("bazel build -c opt --config=windows '//cuda_cute:matmul.pyd'")
  sys.path.append(os.path.join(os.path.dirname(__file__), "../windows/bazel-bin/cuda_cute"))
  if tuple(sys.version_info) > (3, 8):
    ...
    import ctypes
    matmul_lib = ctypes.CDLL(os.path.join(os.path.dirname(__file__), "../windows/bazel-bin/cuda_cute/matmul.pyd"), winmode=0)
